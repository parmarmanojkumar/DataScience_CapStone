...
tm_pre_process <- function(data) {
        ...
        pwdat <-
                read.table(
                        "./00_Data/final/en_US/en_bws.txt",
                        header = FALSE,
                        sep = "\n",
                        strip.white = TRUE
                )
...
contentPart <-
        c(
                sample(contentTweet, length(contentTweet) * percentageData),
                sample(contentBlog, length(contentBlog) * percentageData),
                sample(contentNews, length(contentNews) * percentageData)
        )
corpusPart <- tm_pre_process(contentPart)

#corpus = tm_pre_process(c(contentTweet, contentBlog, contentNews))
corpus <- corpusPart
